...
void STMPE811::initialize()										// Based on AN2807, but adapted for polling io interrupt driven
	...
	// TODO : add workaround for the floating pins which may result in wrong SPI mode
	// try all modes until the version reads a proper 0x811 value..
...
uint8_t STMPE811::read8(uint8_t address)									// See specification section 5.1.1
    ...
    SPI.beginTransaction(SPISettings(1000000, MSBFIRST, SPI_MODE0));		// set the SPI in the correct mode for STMPE
